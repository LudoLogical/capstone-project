...
function buildGrantView(grant: Grant, stageOverrides: Record<string, GrantLifecycleStage>): GrantView {
  const record = INITIATIVE_HILLTOP_WELLNESS.grantRecords.get(grant.id);
  const stage = stageOverrides[grant.id] ?? record?.stage ?? GrantLifecycleStage.NotSaved;
  return {
    grant,
    stage,
...
export function isSavedStage(stage: GrantLifecycleStage): boolean {
  return stage !== GrantLifecycleStage.NotSaved && stage !== GrantLifecycleStage.Unsaved;
}
...
export function useDashboardGroups() {
  ...
  const saved = views.filter((v) => v.stage === GrantLifecycleStage.Saved && !v.hasWritingProgress);
  const awarded = views.filter(
    (v) => v.stage === GrantLifecycleStage.Awarded || v.stage === GrantLifecycleStage.Reported,
  );
  return { inProgress, saved, awarded };
}

export function useAccountSectionsView(): (AccountSection & { factsResolved: AccountSection["facts"] })[] {
