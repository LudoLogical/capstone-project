...
export default function GrantCard({ grant, stage }: { grant: Grant; stage: GrantLifecycleStage }) {
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 240 }}>
          <div
            ...
            role="button"
            tabIndex={0}
            style={{ fontFamily: "var(--font-serif)", fontSize: 20, marginBottom: 5, cursor: "pointer" }}
          >
          ...
          <p style={{ fontSize: 14, lineHeight: 1.5, color: "var(--color-text-muted)", margin: 0, marginBottom: 12 }}>
            {grant.purpose}
          </p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end", flexShrink: 0 }}>
          <div style={{ fontWeight: 700, color: "var(--color-accent-ink-2)", fontSize: 15 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <button onClick={toggleSave} className="btn btn-secondary btn-sm" aria-pressed={saved}>
              {saved ? "★ Saved" : "☆ Save"}
            </button>
            <button onClick={() => navigate(`/grants/${grant.id}`)} className="btn btn-primary btn-sm">
              View grant →
            </button>
          </div>
        </div>
